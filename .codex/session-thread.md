...
- Durable local vector store completed before CSV support: 780 records, 780 embedding rows, 512 dimensions, local `BAAI/bge-small-zh-v1.5`, row-aligned metadata in `shared/indexes/consultant-agent/all-extractable-bge-small-zh-v1-5-20260619/`; it must be rebuilt before runtime retrieval can claim coverage for the 800-card set.
...
- answer trace: 12/12
- durable local vector store: 780 records, 780 embedding rows, local BGE 512-dim embeddings from the pre-CSV all-extractable set
...
- private no-provider retrieval API smoke: record_count 780, label_seed_match_at_1 0.9375, label_seed_match_at_5 1.0, policy_refusal_pass_rate 1.0, workspace forbidden 403, failure_count 0; API still points at the pre-CSV 780-record index
...
- next local build choices: rebuild the durable vector store/API from the 800-card set, actual human-review decisions for locator labels, security-approved staging implementation, or PRD addendum promotion after human/legal review.
